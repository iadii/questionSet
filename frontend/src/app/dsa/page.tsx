...
export default function DSASheetPage() {
  ...
  // Fetch all questions for the selected category without pagination
  const { data: questionsData, isLoading: questionsLoading } = useQuery<PageResponse<Question>>({
    queryKey: ["questions", selectedCategory],
    queryFn: () => apiFetch<PageResponse<Question>>(`/questions?category=${selectedCategory}&page=0&size=500`, { requireAuth: true }),
  });
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div className="flex flex-col gap-4">
              <div className="flex justify-between items-center">
                <div>
                  <h1 className="text-3xl font-bold text-white mb-2">Practice Sheet</h1>
                ...
                <div className="text-right bg-white/5 px-4 py-2 rounded-xl border border-white/10">
                  <div className="text-sm text-gray-400 font-medium">{selectedCategory.replace("_", " ")} Progress</div>
              ...
              <div className="flex gap-4 mt-2 overflow-x-auto pb-1">
                {["DSA", "LLD", "HLD", "CS_FUNDAMENTALS", "BEHAVIORAL"].map(cat => (
                  <button
                    key={cat}
                    onClick={() => {
            ...
            <div className="space-y-4">
              {Object.entries(grouped).length === 0 ? (
                <div className="text-center py-20 bg-white/5 rounded-2xl border border-white/10 border-dashed backdrop-blur-xl">
                  ...
                  <p className="text-gray-400 text-sm">Try adjusting your filters or category to see more results.</p>
                </div>
              ) : (
